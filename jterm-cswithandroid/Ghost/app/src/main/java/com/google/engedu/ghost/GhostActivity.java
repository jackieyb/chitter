...
public class GhostActivity extends AppCompatActivity {
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        if (('A' <= unicode && unicode <= 'Z') || 'a' <= unicode && unicode <= 'z') {
            ...
            if (dictionary.isWord(currentWord)) {
                ((TextView) findViewById(R.id.gameStatus)).setText("Is a complete word");
            }
            computerTurn();
        }
    ...
    private void computerTurn() {
        ...
        if (label.length()>=4){
            ((TextView) findViewById(R.id.gameStatus)).setText("Victory1!");
        }
        String longerWord = dictionary.getAnyWordStartingWith(label.toString());
        if (longerWord!=null){

        }else{
            ((TextView) findViewById(R.id.gameStatus)).setText("Victory2!");
        }
        userTurn = true;
        label.setText(USER_TURN);
    }
